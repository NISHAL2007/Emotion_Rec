...
class VisualOverlay:
    """
    Renders high-quality, color-coded visual overlays, bounding boxes, confidence progress bars,
    and system status messages on live video frames.
    """
    ...
    def draw_face_overlay(self, frame, x, y, w, h, emotion, confidence, probabilities):
        """
        Draws bounding box with corner accents, emotion label banner, and horizontal confidence bar.
        """
        color = self.colors.get(emotion, (180, 180, 180))

        # 1. Draw Corner-Accented Bounding Box
        self._draw_corner_box(frame, x, y, w, h, color)

        # 2. Top Label Banner (Emotion Name & Confidence %)
    ...
    def _draw_corner_box(self, frame, x, y, w, h, color):
        """
        Draws bounding box with stylized corner line segments.
        """
    ...
    def draw_hud(self, frame, fps, face_count, is_low_light=False):
        """
        Draws top HUD bar displaying FPS counter, face count, and low-lighting warning message.
        """
        h, w = frame.shape[:2]
        ...
        # HUD Info Text
        hud_text = f"REAL-TIME EMOTION DETECTOR  |  FPS: {fps:.1f}  |  Faces: {face_count}"
        cv2.putText(frame, hud_text, (15, 24), self.font, 0.55, (240, 240, 240), 1, cv2.LINE_AA)

        # Status / Edge Case Warning Indicators
        if is_low_light:
            warning_text = "! LOW LIGHT DETECTED !"
            cv2.putText(frame, warning_text, (w - 220, 24), self.font, 0.5, (0, 140, 255), 2, cv2.LINE_AA)
        elif face_count == 0:
            status_text = "Searching for faces..."
            cv2.putText(frame, status_text, (w - 210, 24), self.font, 0.5, (160, 160, 160), 1, cv2.LINE_AA)

    @staticmethod
    def check_low_light(frame, threshold=LOW_LIGHT_THRESHOLD):
        """
        Checks if the average frame brightness is below low lighting threshold.
        """
